    ...
    public void childEvent(CuratorFramework curatorFramework, PathChildrenCacheEvent event) throws Exception {
        switch (event.getType()) {
            case CHILD_ADDED:
                ...
                ServiceNodeManage.getInstance().addServicesNode(new String(childData.getData()));
                break;
            case CHILD_REMOVED:
                LOGGER.info(event.getType() + "--" + event.getData().getPath() + "--" + new String(event.getData().getData()));
                ServiceNodeManage.getInstance().removeServicesNode(new String(event.getData().getData()));
                break;
            case CHILD_UPDATED:
                LOGGER.info(event.getType() + "--" + event.getData().getPath() + "--" + new String(event.getData().getData()));
                ServiceNodeManage.getInstance().updateServicesNode(new String(event.getData().getData()));
                break;
            default:
                break;
        }
